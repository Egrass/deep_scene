...
mean_pix = [105.4811, 194.7396, 144.3830];
...
for p=3:round(loop)
    ...
    %im(:,:,2)=imread(PATH);
    A=imread(PATH);
    im(:,:,2)=imresize(A, FINAL_DIM);
    %%NIR
    name4=files4(p).name;
    token = strtok(name,'.');
    ss = DIR4;
    PATH = strcat(ss,name4);
    
    A=imread(PATH);
    im(:,:,3)=imresize(A, FINAL_DIM);
