...
for p=3:round(loop)
    ...
    for i = 1:aug_num
        ...
        if(x>2)
                %%%%%%%%%%%%%%%%%%%IMG%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                immm1=im(:,:,1:3);
                II = imresize(immm1, [FINAL_DIM(1) FINAL_DIM(2)]);
                ...
                im(:,:,1:3)=finImage;
                for c = 1:4
                    finImage(:, :, c) = im(:, :, c) - mean_pix(c);
                end
                augData2 = finImage;

                
                augData=permute(augData2, [2 1 3 4]);
                Img(:,:,:,i+(aug_num*index))=augData;
...
%Img is the data structure with the augmented data
 ...
 data_caffe=zeros([FINAL_DIM(2) FINAL_DIM(1) 4  ((round((loop-2))*(aug_num)))]);
